...
import { GetInsuranceTypes, AddInsuranceType, DeleteInsuranceType } from "../../REST/utilities";
...
export default function InsuranceTypes() {
    ...
    useEffect(() => {
        (async () => {
            const result = await GetInsuranceTypes()
            setData(result.data)
            setMode(0)
        })()
    }, [refresh])
        ...
        <List sx={{ width: '100%', maxWidth: 360, bgcolor: 'background.paper' }}>
            ...
            <ListItem button>
                <ListItemButton onClick={() => handleMode(-1)} dense>
                    <ListItemText primary={`Add Insurance Type`} />
...
function FormDialog({ open = false, data, handleClose, handleSave }) {
    ...
    return <Dialog open={open} onClose={handleClose}>
        <DialogTitle>Insurance Type</DialogTitle>
        <DialogContent>
